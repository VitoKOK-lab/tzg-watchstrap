...
export default function Atelier({inquiriesEnabled=true}:{inquiriesEnabled?:boolean}){
  ...
  return <div className="atelier-edition">
    ...
    <main id="main">
      ...
      <BrandStory/>
      <ProductFilm onExplore={()=>chooseDesign('green')}/>

      <section className="collection-section section-shell" id="collection" aria-labelledby="collection-title"><div className="section-heading"><div><p className="eyebrow">G852 COLLECTION / 01—05</p><h2 id="collection-title">腕間的五種表情。</h2></div><p>從細緻鏈節，到鮮明的色彩排列。<br/>選擇作品，探索材質與定價。</p></div><div className="collection-grid">{designs.map(item=><button key={item.id} className={`design-card ${selected===item.id?'active':''}`} onClick={()=>chooseDesign(item.id)} aria-pressed={selected===item.id}><div className="design-image"><img src={item.image} width="1254" height="1254" loading="lazy" alt={`${item.model} ${item.name}白K SHOPLINE商品實拍`}/><span>{item.number}</span></div><div className="design-caption"><div><p>{item.model}</p><h3>{item.name}</h3></div><ArrowUpRight size={19}/></div></button>)}</div>
